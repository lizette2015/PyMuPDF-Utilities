...
if not tuple(map(int, pymupdf.VersionBind.split("."))) > (1, 21, 1):
...
w = pymupdf.Widget()  # define a field skeleton object for the text
w.rect = pymupdf.Rect(100, 150, 300, 170)
...
w = pymupdf.Widget()  # define field skeleton for the checkbox
w.rect = pymupdf.Rect(100, 100, 120, 120)
w.field_type = pymupdf.PDF_WIDGET_TYPE_CHECKBOX
w.field_name = "my-checkbox"  # use this to identify the field document-wide
w.border_color = pymupdf.pdfcolor["red"]
